    ...
    if (daily == 'DAILY') {
      if (!rRule.contains('BYDAY')) {
        ...
        while (tempCount < recCount ||
          ...
          addDate = _addDaysWithTime(addDate, dailyDayGap, recurrenceStartHour,
        ...
        while (recDateCollection.length < recCount ||
          ...
          addDate = _addDaysWithTime(addDate, 1, recurrenceStartHour,
              recurrenceStartMinute, recurrenceStartSecond);
        }
      }
      ...
      while ((tempCount < recCount && isWeeklySelected) ||
        ...
        addDate = addDate.weekday == DateTime.saturday
            ? _addDaysWithTime(
                addDate,
                ((weeklyWeekGap - 1) * _kNumberOfDaysInWeek) + 1,
                recurrenceStartHour,
                recurrenceStartMinute,
                recurrenceStartSecond)
            : _addDaysWithTime(addDate, 1, recurrenceStartHour,
                recurrenceStartMinute, recurrenceStartSecond);
      }
      ...
      if (byMonthDay == 'BYMONTHDAY') {
        ...
        while (recDateCollection.length < recCount ||
          ...
          final DateTime monthStart = DateTime(
              ...
              recurrenceStartMinute,
              recurrenceStartSecond);
          final DateTime weekStartDate = _addDaysWithTime(
              monthStart,
              -monthStart.weekday,
              recurrenceStartHour,
              recurrenceStartMinute,
          ...
          addDate = _addDaysWithTime(
              weekStartDate,
              (nthWeek * _kNumberOfDaysInWeek) + nthWeekDay,
              recurrenceStartHour,
              recurrenceStartMinute,
              recurrenceStartSecond);
      ...
      if (byMonthDay == 'BYMONTHDAY') {
        ...
        DateTime monthStart = DateTime(addDate.year, monthIndex, 1,
            recurrenceStartHour, recurrenceStartMinute, recurrenceStartSecond);
        DateTime weekStartDate = _addDaysWithTime(
            monthStart,
            -monthStart.weekday,
            recurrenceStartHour,
            recurrenceStartMinute,
        ...
        if ((monthStart.month != addDate.month &&
          ...
          monthStart = DateTime(
              ...
              recurrenceStartMinute,
              recurrenceStartSecond);
          weekStartDate = _addDaysWithTime(
              monthStart,
              -monthStart.weekday,
              recurrenceStartHour,
              recurrenceStartMinute,
    ...
    if (daily == 'DAILY') {
      ...
      while (i < _kNumberOfDaysInWeek && weeklyByDayPos != null) {
        ...
        addDate = addDate.weekday == 6
            ? addDays(
                addDate, ((recProp.interval - 1) * _kNumberOfDaysInWeek) + 1)
            : addDays(addDate, 1);
        i = i + 1;
      }
    } else if (monthly == 'MONTHLY') {
  ...
  if ((recCount > 0 &&
    ...
    while ((count < recCount &&
      ...
      addDate = addDate.weekday == DateTime.saturday
          ? _addDaysWithTime(
              addDate,
              ((recurrenceProperties.interval - 1) * _kNumberOfDaysInWeek) + 1,
              startDate.hour,
              startDate.minute,
              startDate.second)
          : _addDaysWithTime(
...
int _getWeekDay(String weekDay) {
  ...
  final DateTime firstDate =
      addDays(DateTime.now(), -(DateTime.now().weekday - 1));
  final List<String> dayNames = List<int>.generate(
          _kNumberOfDaysInWeek, (int index) => index)
      .map((int value) =>
          DateFormat(DateFormat.ABBR_WEEKDAY).format(addDays(firstDate, value)))
      .toList();
